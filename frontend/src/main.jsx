    ...
    <>
      <aside className={`sidebar ${open ? 'mobile-open' : ''}`}>
        <div className="brand-row">
          ...
          <button className="icon-button mobile-only" onClick={close}><X size={18} /></button>
        </div>
...
function Topbar({ active, apiOk, user, logout, openSidebar }) {
  return (
    <header className="topbar">
      <div className="title-block">
        <button className="icon-button mobile-only" onClick={openSidebar}><Menu size={20} /></button>
        <div>
          <div className="eyebrow">QUANTUM MARKET INTELLIGENCE · FOUNDATION v1.3</div>
          <h1>{active.label}</h1>
          <div className="breadcrumb"><span>QMI</span><ChevronRight size={14}/><span>{active.label}</span></div>
        </div>
      </div>
      <div className="top-actions">
        <div className="search-pill"><Search size={16}/><span>Search modules</span></div>
        <button className="icon-button"><Bell size={18}/></button>
        <span className={apiOk ? 'ok' : 'bad'}>{apiOk ? 'API OK' : 'API OFFLINE'}</span>
        <div className="user-card"><User size={18}/><div><strong>{user?.name || 'Rodri'}</strong><span>{user?.email || user?.role || 'Founder / Investor'}</span></div></div><button className="logout-button" onClick={() => logout()}>Logout</button>
      </div>
    </header>
  );
}
...
function Overview({ portfolio, market, ai, trend, allocation, sectorAllocation, navigate }) {
  return (
    <>
      <div className="grid4">
        <Card title="Portfolio Value" value={money(portfolio?.total_value)} subtitle="SQLite portfolio data" icon={<Wallet size={18} />} />
        <Card title="Total P/L" value={money(portfolio?.total_pl)} subtitle={`${pct(portfolio?.total_pl_pct)} total result`} icon={<Activity size={18} />} />
        <Card title="Open Positions" value={portfolio?.position_count ?? '--'} subtitle={`Largest weight ${pct(portfolio?.largest_position_weight)}`} icon={<LineChart size={18} />} />
        <Card title="AI Layer" value={ai?.status || 'Loading'} subtitle="Architecture prepared" icon={<Brain size={18} />} />
      </div>
      <div className="quick-grid">
        <button onClick={() => navigate('market')}>Open Market</button>
        <button onClick={() => navigate('portfolio')}>Open Portfolio</button>
        <button onClick={() => navigate('ai')}>Open AI Layer</button>
        <button onClick={() => navigate('risk')}>Open Risk</button>
      </div>
      <div className="layout">
        <Panel title="Market Intelligence Trend" subtitle="Foundation analytical signal">
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={trend}><CartesianGrid strokeDasharray="3 3" stroke="#20314d"/><XAxis dataKey="month" stroke="#9db7df"/><YAxis stroke="#9db7df"/><Tooltip/><Area type="monotone" dataKey="value" stroke="#3da5ff" fill="#163d63"/></AreaChart>
          </ResponsiveContainer>
        </Panel>
        <Market market={market}/>
      </div>
      <div className="layout"><Panel title="Portfolio Allocation" subtitle="Position value distribution">
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={allocation}><CartesianGrid strokeDasharray="3 3" stroke="#20314d"/><XAxis dataKey="ticker" stroke="#9db7df"/><YAxis stroke="#9db7df"/><Tooltip/><Bar dataKey="value" fill="#3da5ff" radius={[8,8,0,0]}/></BarChart>
        </ResponsiveContainer>
      </Panel><Panel title="Sector Exposure" subtitle="Allocation by sector">
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={sectorAllocation}><CartesianGrid strokeDasharray="3 3" stroke="#20314d"/><XAxis dataKey="sector" stroke="#9db7df"/><YAxis stroke="#9db7df"/><Tooltip/><Bar dataKey="value" fill="#3da5ff" radius={[8,8,0,0]}/></BarChart>
        </ResponsiveContainer>
      </Panel></div>
    </>
  );
