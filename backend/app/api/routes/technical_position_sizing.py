...
from app.services.technical.position_sizing_service import (
    TechnicalPositionSizingService,
)
...
def get_technical_position_sizing(
    ...
    Converts Technical Risk & Exposure + Action + Decision + Scenario
    into a technical-only allocation framework.

    Does NOT perform final portfolio position sizing.
    """
    ...
    try:
        ...
        result = position_sizing_service.analyze(
            risk_exposure_response=risk_exposure_response,
        ...
        return {
            ...
            "source_risk_engine_id": (
                risk_exposure_response.get("engine_id")
            ),
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "Failed to calculate technical position sizing. "
