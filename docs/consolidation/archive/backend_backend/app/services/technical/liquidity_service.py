...
class LiquidityService:
    ...
    def analyze(
        ...
        liquidity_bias = self._calculate_cluster_bias(
    ...
    def _calculate_cluster_bias(
        self,
        active_clusters: list[dict[str, Any]],
        current_price: float,
    ) -> dict[str, Any]:
        """
        Converts active liquidity clusters into a directional context score.

        Positive score:
            stronger / nearer SSL clusters -> bullish reversal potential.

        Negative score:
            stronger / nearer BSL clusters -> bearish rejection potential.
        ...
        if not active_clusters:
            return {
                ...
                "bsl_pressure": 0.0,
                "ssl_pressure": 0.0,
                "interpretation": "No active institutional liquidity clusters.",
            }
        ...
        for cluster in active_clusters:
            distance = max(0.25, float(cluster["distance_pct"]))
            relevance = float(cluster["decision_relevance"])
            proximity_weight = min(2.5, 4.0 / distance)
            pressure = relevance * proximity_weight

            if cluster["type"] == "BSL":
                bsl_pressure += pressure
            else:
                ssl_pressure += pressure

        total = bsl_pressure + ssl_pressure
        ...
        if total <= 0:
            ...
            score = (
                (ssl_pressure - bsl_pressure)
                / total
                * 100.0
            )
            confidence = min(
                100.0,
                total / max(1.0, len(active_clusters)) / 1.5,
            )
        ...
        if score >= 60:
            state = "STRONG_BULLISH_LIQUIDITY"
        elif score >= 25:
            state = "BULLISH_LIQUIDITY"
        elif score <= -60:
            state = "STRONG_BEARISH_LIQUIDITY"
        elif score <= -25:
            state = "BEARISH_LIQUIDITY"
        else:
            state = "BALANCED"
        ...
        return {
            ...
            "bsl_pressure": round(bsl_pressure, 1),
            "ssl_pressure": round(ssl_pressure, 1),
            "interpretation": (
                "Positive values indicate stronger nearby SSL reversal context; "
                "negative values indicate stronger nearby BSL rejection context. "
                "This is contextual evidence, not a standalone signal."
            ),
        }
